...
export default function MovieDetailsPage() {
  ...
  const relatedMovies = getMoviesByCategory(movie.category).filter((m) => m.id !== movie.id)
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative z-20 h-full flex flex-col justify-end">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pb-12">
            ...
            <div className="max-w-3xl space-y-4">
              <h1 className="text-4xl sm:text-5xl font-bold text-balance">{movie.title[language]}</h1>
              ...
              <div className="flex items-center gap-4 text-sm">
                ...
                <span className="text-muted-foreground capitalize">{t(`category.${movie.category}`)}</span>
              </div>

              <p className="text-lg text-muted-foreground text-pretty">{movie.description[language]}</p>
              ...
              {!canWatch && (
                <div className="mt-4 p-4 bg-accent/20 border border-accent rounded-lg">
                  <p className="text-sm text-accent-foreground">{t("subscription.locked")}</p>
                </div>
              )}
            </div>
          </div>
      ...
      {relatedMovies.length > 0 && (
        <div className="py-12">
          <MovieRow title={t(`category.${movie.category}`)} movies={relatedMovies} />
        </div>
      )}
    </div>
  )
